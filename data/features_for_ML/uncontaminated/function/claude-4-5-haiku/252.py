def list_schedules(
    ...
    if updated is not None:
        filters.append(f"updated:{updated}")
    if name is not None:
        filters.append(f"name:{name}")
    if pipeline_id is not None:
        filters.append(f"pipeline_id:{pipeline_id}")
    if orchestrator_id is not None:
        filters.append(f"orchestrator_id:{orchestrator_id}")
    if active is not None:
        filters.append(f"active:{active}")
    
    filter_string = f" {logical_operator} ".join(filters) if filters else None
